...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        } else if (newBlock.getMinimumTrustLevel() >= this.getMinimumTrustLevel(newBlock.index)) { // If the trust level of the proof-of-work challenge is correct
            console.error(`Invalid proof-of-trust level: expected '${newBlock.getMinimumTrustLevel()}' to be smaller than '${this.getMinimumTrustLevel(newBlock.index)}'`);
            throw new BlockAssertionError(`Invalid proof-of-trust difficulty: expected '${newBlock.getMinimumTrustLevel()}' be smaller than '${this.getMinimumTrustLevel()}'`);
        }
