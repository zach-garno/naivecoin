...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
        ...
        const thread = spawn(function (input, done) {
            ...
            done(Miner.proveTrustFor(Block.fromJson(input.jsonBlock), null));
        });
        ...
        console.log('thread.send info');
        console.log(__dirname);
        console.log(this.logLevel);
        console.log(baseBlock);
        console.log(this.blockchain);
        console.log(this.blockchain.getMinimumTrustLevel());
        console.log('thread.send');
        thread.send({
            __dirname: __dirname,
            logLevel: this.logLevel,
            jsonBlock: baseBlock,
            difficulty: this.blockchain.getMinimumTrustLevel()
        });
    ...
    revalidate(rewardAddress, feeAddress) {
        //TODO all of this
    }
    ...
    static proveTrustFor(jsonBlock, peers) {
        let block = Block.fromJson(jsonBlock);
        // let validChecks = null;
        // let trust = 0;
        // block.timestamp = new Date().getTime() / 1000;
        // // Loop 
        // do {
        //     block.nonce = 5//getRandomInt(100000);
        //     block.hash = block.toHash();//may not be necessary
        //     validChecks = block.getDifficulty();//left this in for compatability
        //     //TODO api call a revalidators node sending the block
        //     //TODO increment the trust based on the responce of the api call
        // } while (maxTrustLevel >= trust);
        // console.info(`Block trusted: time '${process.hrtime(block.timestamp)[0]} sec' trust '${trust}' hash '${block.hash}' nonce '${block.nonce}'`);
        return block;
    }
}
