...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/revalidator/revalidate', (req, res) => {
            miner.revalidate(req.body.rewardAddress, req.body['feeAddress'] || req.body.rewardAddress)
                .then(() => {
                    res.status(200).send();//TODO what to send back bool?
                })
                .catch((ex) => {
                    //TODO post a new transaction of the bad block
                });
        });
